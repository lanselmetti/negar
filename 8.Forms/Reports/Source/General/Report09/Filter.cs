...
namespace Sepehr.Forms.Reports.General.Report09
{
    /// <summary>
    ...
    public partial class frmFilter : Form
    {
        ...
        private void BGWorker_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            {
                _ReportDataTable.Columns["RegisterDate"].ReadOnly = false;
                _ReportDataTable.Columns["TransDateTime"].ReadOnly = false;
                for (Int16 i = 0; i < _ReportDataTable.Rows.Count; i++)
                {
                    PersianDate PDate = Convert.ToDateTime(_ReportDataTable.Rows[i]["RegisterDateEn"]).ToPersianDate();
                    _ReportDataTable.Rows[i]["RegisterDate"] = PDate.Year + "/" + PDate.Month + "/" + PDate.Day + " - " +
                        PDate.Hour + ":" + PDate.Minute + ":" + PDate.Second;
                    PDate = Convert.ToDateTime(_ReportDataTable.Rows[i]["TransDateTimeEn"]).ToPersianDate();
        ...
        private Boolean GenerateReportString()
        {
            ...
            ReportSelectString.Append("SELECT [Tbl3].[PatientID] , " +
                "[FullName] = (ISNULL([Tbl3].[FirstName] + ' ' , '') + [Tbl3].[LastName] ), " +
                "[Tbl1].[RegisterDate] AS [RegisterDateEn], " +
                "'1200/12/31 - 12:59:59' AS [RegisterDate] , " +
            ...
            if (!_CalcReportRowCount) ReportSelectWhereString.Append(" ORDER BY [Tbl2].[OccuredDate];");
            #endregion
