...
namespace Sepehr.Settings.Patients
{
    ...
    public partial class frmCities : Form
    {
        ...
        #region IQueryable<City> _DataSource
        /// <summary>
        /// فیلد منبع اطلاعاتی فرم
        /// </summary>
        private IQueryable<City> _DataSource;
        #endregion
        ...
        private void cBoxWithInActives_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                dgvData.DataSource = (_DataSource.Where(FilteredData => FilteredData.IsActive));
            }
        }
        #endregion
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            DialogResult Dr = PMBox.Show("آیا مایلید شهر زیر حذف گردد:\n\"" +
                dgvData.Rows[RowIndex].Cells[1].Value +
                "\"\nبا حذف این شهر ، اطلاعات شهر سكونت كلیه بیمارانی كه این شهر برای آنها ثبت شده حذف خواهد شد.",
            ...
            if (Dr == DialogResult.Yes)
                ...
                {
                    const String ErrorMessage = "امكان حذف شهر انتخاب شده از بانك وجود ندارد.\n" +
        ...
        private void SetControlsToolTipTexts()
        {
            ...
            TooltipText = ToolTipManager.GetText("cBoxWithInActivesPatCities", "IMS");
            ...
            #region btnAdd
            TooltipText = ToolTipManager.GetText("btnAddPatCity", "IMS");
        ...
        private Boolean FillDataSource()
        {
            ...
            try
            {
                _DataSource = Negar.DBLayerPMS.Manager.DBML.Cities.OrderBy(City => City.Name);
                States = Negar.DBLayerPMS.Manager.DBML.States.OrderBy(State => State.Name).ToList();
                if (States.Count == 0)
                {
                    PMBox.Show("استانی برای ثبت شهر وجود ندارد! لطفاً ابتدا یك استان ثبت نمایید.", "خطا!",
                        MessageBoxButtons.OK, MessageBoxIcon.Error); return false;
                }
                _DefaultRowValue = States.Select(FirstRow => FirstRow.ID).OrderBy(Data => Data).First();
            }
            #region Catch
            catch (Exception Ex)
            {
                const String ErrorMessage =
                    "امكان خواندن اطلاعات شهرها از بانك وجود ندارد.\n" +
                    "موارد زیر را بررسی نمایید:\n" +
                    "1. آیا ارتباط شما با بانك اطلاعات برقرار است و شبكه متصل می باشد؟";
                PMBox.Show(ErrorMessage, "خطا!", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            #endregion
            dgvData.DataSource = _DataSource;
            ColState.DataSource = States;
            ColState.DisplayMember = "Name";
