...
namespace Sepehr.Settings.Referrals
{
    ...
    public partial class frmAddinColsMultiChoiceFieldItems : Form
    {
        ...
        #region List<RefAdditionalDataItemsColCover> _CurrentFieldItems
        /// <summary>
        /// لیست آیتم های تحت پوشش فیلد چندگزینه ای پویا جاری
        /// </summary>
        private List<RefAdditionalDataItemsColCover> _CurrentFieldItems;
        #endregion
        ...
        {
            Int16 ItemID = ((RefAdditionalDataItemsColCover)dgvSelectedItems.Rows[e.RowIndex].DataBoundItem).ItemIX;
            List<RefAdditionalDataItem> TempData = DBLayerIMS.Referrals.RefAddinDataItemsList;
            if (TempData == null) { Close(); return; }
            e.Value = DBLayerIMS.Referrals.RefAddinDataItemsList.Where(Data => Data.ID == ItemID).First().Title;
        }
        #endregion
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            Int16 SelectedItem = ((RefAdditionalDataItem)dgvItems.SelectedRows[0].DataBoundItem).ID;
            foreach (DataGridViewRow row in dgvSelectedItems.Rows)
                if (((RefAdditionalDataItemsColCover)row.DataBoundItem).ItemIX == SelectedItem) return;
            RefAdditionalDataItemsColCover NewItem = new RefAdditionalDataItemsColCover();
            NewItem.ColumnIX = _CurrentFieldID;
            NewItem.ItemIX = SelectedItem;
            DBLayerIMS.Manager.DBML.RefAdditionalDataItemsColCovers.InsertOnSubmit(NewItem);
        ...
        {
            DBLayerIMS.Referrals.RefAddinDataItemsList = null;
            List<RefAdditionalDataItem> TempAddinItemsList = DBLayerIMS.Referrals.RefAddinDataItemsList;
            ...
            _CurrentFieldItems = DBLayerIMS.Referrals.GetRefAddinDataItemsColCovers(_CurrentFieldID);
