...
namespace Sepehr.Settings.PACSIntegration
{
    ...
    /// </summary>
    public partial class frmStudiesServicesGrouping : Form
    {

        #region Fields

        #region List<SP_SelectServicesInGroupsResult> _ServicesStudies
        /// <summary>
        /// فیلد منبع اطلاعاتی فرم
        /// </summary>
        private List<ServiceStudy> _ServicesStudies;
        #endregion
        ...
        /// سازنده پیش فرض فرم
        /// </summary>
        public frmStudiesServicesGrouping()
        {
            InitializeComponent();
            dgvData.AutoGenerateColumns = false;
            DBLayerIMS.PACS.Modalities = null;
            DBLayerIMS.PACS.ServiceModalities = null;
            if (!FillServicesDataSource() || !FillStudiesDataSource()) { Close(); return; }
            ShowDialog();
        }
        #endregion
        ...
        private void Form_Shown(object sender, EventArgs e)
        {
            ...
            dgvData.DataSource = _ServicesStudies.Where(
                Data => Data.StudyIX == Convert.ToInt16(cboGroups.SelectedValue)).ToList();
        }
        #endregion
        ...
        private void dgvData_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            try
            {
                if (e.ColumnIndex == ColCode.Index)
                    e.Value = ((ServiceStudy)dgvData.Rows[e.RowIndex].DataBoundItem).ServicesList.Code;
                else if (e.ColumnIndex == ColServiceName.Index)
                    e.Value = ((ServiceStudy)dgvData.Rows[e.RowIndex].DataBoundItem).ServicesList.Name;
            }
            #region Catch
            catch (Exception Ex)
            {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            List<ServiceStudy> Items;
            try
            {
                Items = DBLayerIMS.Manager.DBML.ServiceStudies.
                    Where(Data => Data.StudyIX == Convert.ToInt16(cboGroups.SelectedValue)).ToList();
            }
            #region Catch
            catch (Exception Ex)
            {
            ...
            foreach (Int16 ServiceID in AddedServicesID)
            {
                ...
                {
                    ServiceStudy NewItem = new ServiceStudy();
                    NewItem.StudyIX = Convert.ToInt16(cboGroups.SelectedValue);
                    NewItem.ServiceIX = CurrentServiceID;
                    DBLayerIMS.Manager.DBML.ServiceStudies.InsertOnSubmit(NewItem);
                }
            }
            if (!DBLayerIMS.Manager.Submit())
            {
                const String ErrorMessage =
                    "امكان افزودن خدمات قبلی عضو اندام انتخاب شده به اندام جاری وجود ندارد.\n" +
                    "موارد زیر را بررسی نمایید:\n" +
                    "1. آیا ارتباط شما با بانك اطلاعات برقرار است و شبكه متصل می باشد؟";
                PMBox.Show(ErrorMessage, "خطا!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Close(); return;
            ...
            dgvData.DataSource = _ServicesStudies.
                Where(Data => Data.StudyIX == Convert.ToInt16(cboGroups.SelectedValue)).ToList();
            TopMost = true;
            Select();
            Activate();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            DialogResult Dr = PMBox.Show("آیا مایلید خدمات انتخاب شده از اندام جاری حذف گردد؟\n" +
            ...
            foreach (DataGridViewRow row in dgvData.SelectedRows)
                DBLayerIMS.Manager.DBML.ServiceStudies.DeleteAllOnSubmit(
                    DBLayerIMS.Manager.DBML.ServiceStudies.
                        Where(Data => Data.StudyIX == Convert.ToInt16(cboGroups.SelectedValue) &&
                            Data.ServiceIX == ((ServiceStudy)row.DataBoundItem).ServiceIX));
            // ReSharper restore AccessToModifiedClosure
            dgvData.DataSource = null;
            dgvData.Refresh();
            if (!DBLayerIMS.Manager.Submit())
            {
                const String ErrorMessage = "امكان حذف خدمت انتخاب شده از اندام جاری وجود ندارد.\n" +
                    "موارد زیر را بررسی نمایید:\n" +
                    "1. آیا ارتباط شما با بانك اطلاعات برقرار است و شبكه متصل می باشد؟";
        ...
        #region Boolean FillStudiesDataSource()
        /// <summary>
        /// تابع خواندن اطلاعات گروه های خدمات از بانك
        /// </summary>
        private Boolean FillStudiesDataSource()
        {
            List<Study> GroupsDataSource = DBLayerIMS.PACS.Studies;
            if (GroupsDataSource == null) return false;
            if (GroupsDataSource.Count == 0)
            {
                PMBox.Show("اندامی برای مدیریت گروه بندی خدمات ثبت نگردیده است!\n" +
                    "لطفاً ابتدا گروهی را تعریف نمایید.", "خطا!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            cboGroups.DataSource = GroupsDataSource;
            return true;
        }
        #endregion
        ...
        /// <summary>
        /// تابع تكمیل اطلاعات خدمات تحت پوشش اندام
        /// </summary>
        private Boolean FillServicesDataSource()
        {
            DBLayerIMS.PACS.ServiceStudies = null;
            _ServicesStudies = DBLayerIMS.PACS.ServiceStudies;
            if (_ServicesStudies == null) return false;
            return true;
        }
        #endregion
