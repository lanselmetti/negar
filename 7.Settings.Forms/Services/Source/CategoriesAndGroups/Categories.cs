...
namespace Sepehr.Settings.Services
{
    ...
    public partial class frmCategories : Form
    {
        ...
        #region IQueryable<ServicesCategories> _DataSource
        /// <summary>
        /// فیلد منبع اطلاعاتی فرم
        /// </summary>
        private IQueryable<ServicesCategories> _DataSource;
        #endregion
        ...
        private void dgvData_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            if (e.ColumnIndex == Column2.Index)
            {
                foreach (DataGridViewRow row in dgvData.Rows)
                {
                    ...
                    {
                        PMBox.Show("طبقه بندی وارد شده تكراریست! لطفاً مقدار وارد شده را بررسی نمایید.", "خطا!",
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            DialogResult Dr = PMBox.Show("آیا مایلید ردیف زیر حذف گردد:\n\"" +
                ...
                "\"\nبا حذف این طبقه بندی ، طبقه بندی كلیه خدماتی كه این طبقه بندی برای آنها ثبت شده حذف خواهد شد.",
                "هشدار!", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2);
            if (Dr == DialogResult.Yes)
                try { dgvData.Rows.Remove(dgvData.Rows[RowIndex]); _IsGridValuesChanged = true; }
                #region Catch
                catch (Exception Ex)
                {
                    const String ErrorMessage =
                        "امكان حذف طبقه بندی از بانك وجود ندارد.\n" +
        ...
        private void btnApply_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dgvData.Rows)
            {
                ...
                if (!CheckedItems.Contains(row.Cells[Column2.Index].Value.ToString().Trim()))
                {
                    ...
                    foreach (DataGridViewRow TempRow in dgvData.Rows)
                        if (!TempRow.IsNewRow && // ردیف جدید نباشد
                            ...
                            TempRow.Cells[Column2.Index].Value != null && // مقدار آن تهی نباشد
                            // اگر هر دو ردیف برابر باشند
                            TempRow.Cells[Column2.Index].Value.ToString().ToLower() ==
                            row.Cells[Column2.Index].Value.ToString().ToLower())
                        {

                            PMBox.Show("در بین طبقه بندی های وارد شده ، نام گروه تكراری وجود دارد!\n" +
                                "لطفاً اصلاح نمایید.", "خطا!",
                                MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
                            row.Cells[Column2.Index].Selected = true;
                            return;
                        }
                    // افزودن ردیف غیر تكراری به لیست آیتم های چك شده
                    CheckedItems.Add(row.Cells[Column2.Index].Value.ToString().Trim());
                }
                #endregion
            }
            #endregion
        ...
        private void SetControlsToolTipTexts()
        {
            ...
            String TooltipText = ToolTipManager.GetText("cBoxWithInActivesServiceCat", "IMS");
            ...
            #region btnAdd
            TooltipText = ToolTipManager.GetText("btnAddServiceCat", "IMS");
        ...
        {
            try { _DataSource = DBLayerIMS.Manager.DBML.ServicesCategories.OrderBy(Category => Category.Name); }
            #region Catch
            catch (Exception Ex)
            {
                const String ErrorMessage =
                    "امكان خواندن اطلاعات طبقه بندی های خدمات از بانك وجود ندارد.\n" +
                    "موارد زیر را بررسی نمایید:\n" +
                    "1. آیا ارتباط شما با بانك اطلاعات برقرار است و شبكه متصل می باشد؟";
                PMBox.Show(ErrorMessage, "خطا!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                LogManager.SaveLogEntry("Sepehr", "Services Settings", Ex.Message + "\n" + Ex.StackTrace, EventLogEntryType.Error);
