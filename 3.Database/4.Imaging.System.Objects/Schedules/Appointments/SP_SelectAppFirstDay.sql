USE ImagingSystem;
GO
IF OBJECT_ID ( 'Schedules.SP_SelectAppFirstDay', 'P' ) IS NOT NULL
    DROP PROCEDURE [Schedules].[SP_SelectAppFirstDay];
GO
-- @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
-- روالی برای پیدا كردن اولین روز یك برنامه نوبت دهی با توجه به ایام تعطیل
-- @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
CREATE PROCEDURE [Schedules].[SP_SelectAppFirstDay]
@AppID SMALLINT , 
...
AS	
	...
	AND [OccuredDateTime] NOT IN 
		(SELECT [HolidayDate] FROM [ImagingSystem].[Schedules].[Holidays] AS [Tbl2] 
		WHERE [Tbl2].[ApplicationIX] = [Tbl1].[ApplicationIX])
		ORDER BY [Tbl1].[OccuredDateTime]);
GO
-- @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
--DECLARE @Value SMALLDATETIME
--EXEC [Schedules].[SP_SelectAppFirstDay] 83 , @Value OUTPUT
--SELECT @Value
-- @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@