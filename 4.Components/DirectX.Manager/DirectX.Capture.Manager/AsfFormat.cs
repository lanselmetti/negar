...
namespace Negar.DirectX.Capture.Manager
{
    ...
    {
        #region AsfFormatSelection enum
        ...
        private const Int32 MAXLENPROFDESC = 512;
        private const Int32 MAXLENPROFNAME = 100;
        // Windows Media Video 8 for Broadband (PAL, 700 Kbps)
        // Windows Media Audio 8 for Dial-up Modem (CD quality, 64 Kbps)
        private readonly Guid DefaultAudioProfile = WMData.WMProfile_V80_64StereoAudio;
        private readonly Guid DefaultVideoProfile = WMData.WMProfile_V80_700PALVideo;
        ...
        public WMProfileData this[Int32 index]
        {
        ...
        public bool GetCurrentAsfAVInfo(out bool captureAudio, out bool captureVideo)
        {
            ...
            if (CurrentProfile != null)
            {
                ...
                else if ((CurrentProfile.Filename != null) && (CurrentProfile.Filename.Length > 0))
                {
        ...
        /// <returns></returns>
        public Guid GetAsfItemGuid(Int32 item)
        {
        ...
        public bool GetProfileFormatInfo(AsfFormatSelection avformat)
        {
            ...
            try
            {
                ...
                for (Int32 p = 0; (p < nbrProfiles) && (hr >= 0); p++)
                {
                    ...
                    if (hr >= 0)
                    {
                        var profile2 = profile as IWMProfile2;
        ...
        public bool AddProfileItem(AsfFormatSelection avformat, IWMProfile profile, Guid guid, string filename)
        {
            ...
            try
            {
                var profileName = new StringBuilder(MAXLENPROFNAME);
                var profileDescription = new StringBuilder(MAXLENPROFDESC);
                Int32 profileNameLen = MAXLENPROFNAME;
                Int32 profileDescLen = MAXLENPROFDESC;
                Int32 streamCount = 0;
                bool audio = false;
                bool video = false;
                Int32 audioBitrate = 0;
                Int32 videoBitrate = 0;
                profileName.Length = 0;
                profileDescription.Length = 0;

                Int32 hr = profile.GetName(profileName, ref profileNameLen);
#if DEBUG
                if (hr < 0)
                {
                ...
                if ((streamCount > 0) && (hr >= 0))
                {
                    ...
                    for (short i = 1; (i <= streamCount) && (hr >= 0); i++)
                    {
                        ...
                        if ((hr >= 0) && (streamConfig != null))
                        {
                            ...
                            if (hr >= 0)
                            {
                                ...
                                else if (streamGuid == MediaType.Audio)
                                {
                                    audio = true;
                    ...
                    if (hr >= 0)
                    {
                        var profileInfo = new WMProfileData(
                            guid, profileName.ToString(), profileDescription.ToString(), audioBitrate, videoBitrate,
                            audio, video);
        ...
        public bool ConfigProfileFromFile(IBaseFilter asfWriter, string filename)
        {
            ...
            if ((filename != null) && (File.Exists(filename)))
            {
                ...
                if (hr >= 0)
                {
                    ...
                    var configWriter = (IConfigAsfWriter) asfWriter;
        ...
        /// <returns></returns>
        public bool UpdateAsfAVFormat(IBaseFilter asfWriter, Int32 asfItem)
        {
            if (asfWriter != null)
            {
                var lConfig = asfWriter as IConfigAsfWriter;
                Int32 hr = 0;
                ...
                if (asfItem >= 0)
                {
                    ...
                    if (guid != Guid.Empty)
                    {
                        ...
                        {
                            MessageBox.Show(
                                "Problems with updating Windows Media audio/video format information via Guid.");
                            return false;
                        }
                        else
                        {
                    ...
                    else if (this[asfItem].Filename.Length > 0)
                    {
                        if (!ConfigProfileFromFile(asfWriter, this[asfItem].Filename))
                        {
                            MessageBox.Show(
                                "Problems with updating Windows Media audio/video format information via prx file.");
                        }
                        else
                        {
                            CurrentAsfItem = asfItem;
