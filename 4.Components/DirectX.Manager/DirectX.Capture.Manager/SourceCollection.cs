...
namespace Negar.DirectX.Capture.Manager
{
    ...
    public class SourceCollection : CollectionBase, IDisposable
    {
        ...
        public Source this[Int32 index]
        {
...
//#endif
                                                ...
                                                bool isVideoDevice)
        {
            ...
            for (Int32 cOut = 0; cOut < numOutPins; cOut++)
            {
                // Loop through input pins
                for (Int32 cIn = 0; cIn < numInPins; cIn++)
                {
                    ...
                    if (hr == 0)
                    {
                        ...
                        // Add it to the list
                        var source = new CrossbarSource(crossbar, cOut, cIn, relatedInputPin, connectorType);
                        sources.Add(source);
                    }
                }
            }
            ...
            while (refIndex < sources.Count)
            {
                ...
                for (Int32 c = 0; c < sources.Count; c++)
                {
        ...
        protected ArrayList findAudioSources(ICaptureGraphBuilder2 graphBuilder, IBaseFilter deviceFilter)
        {
            ...
            if (audioInputMixer != null)
            {
                ...
                IEnumPins pinEnum;
                Int32 hr = deviceFilter.EnumPins(out pinEnum);
                pinEnum.Reset();
