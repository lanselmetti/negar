...
namespace Sepehr.Forms
{
    ...
    internal partial class frmManageServInterCode : Office2007Form
    {
        ...
        {
            if (e.ColumnIndex != ColICode.Index || String.IsNullOrEmpty(e.FormattedValue.ToString())) return;
            String EnteredText = e.FormattedValue.ToString();
            foreach (Char c in EnteredText)
            {
                if (Char.IsDigit(c) && EnteredText.Count() >= 7) continue;
                PMBox.Show("تنها وارد كردن عبارت هفت رقمی عددی مجاز است!", "اخطار!", MessageBoxButtons.OK);
                e.Cancel = true;
                break;
            }
        }
        ...
        {
            List<SP_SelectServicesListResult> DataSource = DBLayerIMS.Services.ServicesList;
            if (DataSource == null) return false;
            dgvData.DataSource = DataSource.Where(Data => Data.IsActive).OrderBy(Data => Convert.ToInt32(Data.Code)).ToList();
